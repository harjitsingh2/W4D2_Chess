...
RSpec.describe "Goals", type: :system do
  ...
  describe 'Creating a goal (hint: make sure your capitalization is exact!)' do
    context 'when logged in' do
      ...
      it 'displays a link to "Add New Goal" only if user is on their own show page' do
        visit user_url(get_jack)
        expect(page).to have_link('Add New Goal', href: new_goal_url)

        register("jasmine")
        visit user_url(get_jack)
        expect(page).not_to have_link('Add New Goal', href: new_goal_url)
      end
      ...
      context 'on a failed save' do
        ...
        it 'prefills previously input information and still allows for a successful save' do
          expect(find_field('Name').value).to eq('Make a Pizza')
          expect(find_field('False', checked: true))
          fill_in 'Details', with: 'Mushroom Pizza'
          click_button 'Create Goal'
          expect(page).to have_content 'Make a Pizza'
        end
