...
RSpec.describe "Goals", type: :request do
  ...
  describe 'POST /goals (#create)' do
    context 'when logged in' do
      ...
      context 'with invalid params' do
        it 'appropriately stores error messages for display and returns to the "Create New Goal" page' do
          post goals_url, params: { goal: { name: 'this is an invalid goal', status: 'false' } }
          expect(response.body).to include("Add a New Goal")
          expect(flash.now[:errors]).to eq(["Details can't be blank"])
        end
      end

      context 'with valid params' do
        it 'creates the goal and redirects to the user\'s show page' do
          post goals_url, params: { goal: { name: 'Exercise', details: 'do it', status: 'true' } }
          expect(response).to redirect_to(user_url(jack))
    ...
    context 'when logged out' do
      it 'redirects to the login page' do
        post goals_url, params: { user_id: jasmine.id, goal: { name: 'Exercise Again!', details: 'Climb that mountain!', status: 'true' } }
  ...
  describe 'DELETE /goals/:id (#destroy)' do
    context 'when logged in as the goal\'s owner' do
      ...
      it 'removes the goal and stays on the user show page' do
    ...
    context 'when NOT logged in as the goal\'s owner' do
      ...
      it 'does not remove the goal and redirects to the user index' do
        delete goal_url(jasmine_goal), params: { id: jasmine_goal.id }
